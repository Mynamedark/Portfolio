  ...
  const certifications = [
    {
      ...
      verificationUrl: "#",
      file: "/certificates/Course_Completion_Certificate_Intermediate.pdf"
    },
    {
      id: 1,
      title: "CSI Linux",
      ...
      verificationUrl: "#",
      file: "/certificates/CSIL-CI_Certification.pdf"
    },
    {
      id: 2,
      ...
      verificationUrl: "#",
      file: "/certificates/Introduction to Dark Web Operations-course.pdf"
    },
    {
      id: 3,
      ...
      verificationUrl: "#",
      file: "/certificates/cybrary-cert-digital-forensics-basics.pdf"
    }
  ];

  const isPDF = (file: string) => file.toLowerCase().endsWith(".pdf");
  const isImage = (file: string) => /\.(png|jpg|jpeg|webp)$/i.test(file);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {certifications.map((cert, index) => (
              <SpotlightCard
                key={cert.id}
                index={index}
                className="p-8 md:p-12 rounded-[2.5rem] bg-card/30 backdrop-blur-xl border border-border hover:border-primary/50 transition-all duration-500 flex flex-col h-full shadow-2xl group overflow-hidden"
              >
                <div className="flex items-start justify-between mb-10 relative z-10">
                  <div className={`w-20 h-20 rounded-2xl ${cert.color} flex items-center justify-center border border-white/10 shadow-lg`}>
                    <cert.icon className="w-10 h-10 text-white" />
                  </div>
                  <span className="text-sm font-mono uppercase tracking-widest text-primary bg-primary/10 px-4 py-1.5 rounded-full border border-primary/20 font-display">
                    {cert.issuedDate}
                  </span>
                </div>

                <div className="flex-grow space-y-8 relative z-10">
                  <p className="text-xs font-bold uppercase tracking-wider font-display">{cert.category}</p>
                  ...
                  {cert.file ? (
                    isImage(cert.file) ? (
                      ...
                      // PDF PREVIEW
                      <div className="p-6 rounded-2xl border border-border bg-black/20 flex flex-col items-center gap-5">
                        <FileText className="w-16 h-16 text-primary" />
                        <p className="text-muted-foreground text-center">{cert.title} (PDF)</p>
                        <div className="flex gap-4">
                          ...
                          <a
                            href={cert.file}
                            download
                            className="px-5 py-2 rounded-xl border border-primary text-primary font-bold"
                          >
                            Download
                          </a>
                        </div>
      ...
      <section className="py-48 relative z-10">
        <div className="container px-4 mx-auto text-center max-w-5xl">
          <motion.div
            ...
            viewport={{ once: true }}
            variants={revealVariants}
            className="p-16 rounded-[3.5rem] border border-border bg-card/20 backdrop-blur-xl shadow-2xl"
          >
            <h3 className="text-5xl md:text-8xl font-bold mb-10">Defensible <br /><span className="text-muted-foreground/40 italic">Findings.</span></h3>
            <p className="text-2xl text-muted-foreground mb-10 leading-relaxed">
              Every investigation I conduct is supported by technical expertise and verified methodology.
            </p>
            <div className="inline-flex items-center gap-6 px-12 py-6 rounded-full bg-card/30 border border-border backdrop-blur-md shadow-xl">
              ...
              <span className="text-2xl font-bold text-foreground">System Integrity: 100% Verified</span>
            </div>
          </motion.div>
        </div>
      </section>
